...
import com.example.umpbizgo.Customer.PaymentFragment;
...
public class CompleteOrderFragment extends Fragment {
    ...
    private void BackToOrderPage() {
        ...
        ViewUnshippedOrdersFragment fragordercomplete = new ViewUnshippedOrdersFragment();
    ...
    private void ConfirmOrder() {
        ...
        DatabaseReference orderReference = FirebaseDatabase.getInstance().getReference().child("Unpaid Orders");
        ...
        orderReference.child(orderKey).updateChildren(orderMap).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()){
                                    ...
                                    PaymentFragment fragcompleteorder = new PaymentFragment();
                                    Bundle bundle =new Bundle();
                                    bundle.putString("oid",orderKey);
                                    bundle.putString("totalamount",totalAmount);
                                    bundle.putString("name",nameEditText.getText().toString());
                                    bundle.putString("phonenumber", phoneEditText.getText().toString());
                                    bundle.putString("homeaddress", homeEditText.getText().toString());
                                    bundle.putString("cityaddress", cityEditText.getText().toString());
                                    ...
                                    bundle.putString("productImage",imageUrl);
                                    bundle.putString("date", saveCurrentDate);
                                    bundle.putString("time", saveCurrenttime);
                                    bundle.putString("state","Not Shipped");
                                    fragcompleteorder.setArguments(bundle);
